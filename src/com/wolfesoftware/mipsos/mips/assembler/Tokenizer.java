...
public class Tokenizer
{
	...
	private static Token.TokenBase tokenizeRegister(final String src, final int i) throws TokenizingException
	{
		...
		else
		{
			...
			else if (regName.equals("AT")) regNum = 1;
			else if (regName.equals("V0")) regNum = 2;
			else if (regName.equals("V1")) regNum = 3;
			else if (regName.equals("A0")) regNum = 4;
			else if (regName.equals("A1")) regNum = 5;
			else if (regName.equals("A2")) regNum = 6;
			else if (regName.equals("A3")) regNum = 7;
			else if (regName.equals("T0")) regNum = 8;
			else if (regName.equals("T1")) regNum = 9;
			else if (regName.equals("T2")) regNum = 10;
			else if (regName.equals("T3")) regNum = 11;
			else if (regName.equals("T4")) regNum = 12;
			else if (regName.equals("T5")) regNum = 13;
			else if (regName.equals("T6")) regNum = 14;
			else if (regName.equals("T7")) regNum = 15;
			else if (regName.equals("S0")) regNum = 16;
			else if (regName.equals("S1")) regNum = 17;
			else if (regName.equals("S2")) regNum = 18;
			else if (regName.equals("S3")) regNum = 19;
			else if (regName.equals("S4")) regNum = 20;
			else if (regName.equals("S5")) regNum = 21;
			else if (regName.equals("S6")) regNum = 22;
			else if (regName.equals("S7")) regNum = 23;
			else if (regName.equals("T8")) regNum = 24;
			else if (regName.equals("T9")) regNum = 25;
			else if (regName.equals("K0")) regNum = 26;
			else if (regName.equals("K1")) regNum = 27;
			else if (regName.equals("GP")) regNum = 28;
			else if (regName.equals("SP")) regNum = 29;
			else if (regName.equals("FP")) regNum = 30;
			else if (regName.equals("RA")) regNum = 31;
			else throw new TokenizingException(i + 1, PRMT_UnrecognizedRegisterName);
		}
...
//		int lineLength = src.length();
		...
		// check for hex
		if (numStart + 1 < src.length() &&
		    (c == '0' &&
		     src.charAt(numStart + 1) == 'x'))
		{
			// hex
			...
			return new Token.LiteralLong(
					( negative ? -1 : 1 ) * Long.parseLong(src.substring(numStart, j), 16), i, j);
		}
		else
		{
			// dec
			...
			{
				// double
				return new Token.LiteralDouble(
						(negative ? -1.0 : 1.0) * Double.parseDouble(src.substring(numStart, j)), i, j);
			}
			else
			{
				// long
				return new Token.LiteralLong(
						(negative ? -1 : 1) * Long.parseLong(src.substring(numStart, j)), i, j);
			}
		}
	}
	...
	private static Token.TokenBase tokenizeString(final String src, final int i) throws TokenizingException
	{
		...
		while (j < src.length())
		{
			...
			if (escaping)
			{
				...
				case  't': string += '\t'; break;
				default: throw new TokenizingException(j + 1, PRMT_UnrecognizedEscapeSequence);
				}
				escaping = false;
			}
			else if (cj == '\\')
	...
	private static Token.TokenBase tokenizePunct(final String src, final int i)
	{
		...
		switch (src.charAt(i))
		{
		case ',': punct = Token.PunctClass.PunctEnum.COMMA;         break;
		case ':': punct = Token.PunctClass.PunctEnum.COLON;         break;
		case '(': punct = Token.PunctClass.PunctEnum.OPEN_PARENTH;  break;
		case ')': punct = Token.PunctClass.PunctEnum.CLOSE_PARENTH; break;
		default: return null;
		}
	...
	private static Token.TokenBase tokenizeInstrOrLabel(final String src, final int i)
	{
		...
		if      (tokenUpper.equals("ADD"))     instr = Token.InstrName.InstrEnum.ADD;
		else if (tokenUpper.equals("ADDI"))    instr = Token.InstrName.InstrEnum.ADDI;
		else if (tokenUpper.equals("AND"))     instr = Token.InstrName.InstrEnum.AND;
		else if (tokenUpper.equals("ANDI"))    instr = Token.InstrName.InstrEnum.ANDI;
		else if (tokenUpper.equals("BEQ"))     instr = Token.InstrName.InstrEnum.BEQ;
		else if (tokenUpper.equals("BNE"))     instr = Token.InstrName.InstrEnum.BNE;
		else if (tokenUpper.equals("BREAK"))   instr = Token.InstrName.InstrEnum.BREAK;
		else if (tokenUpper.equals("DIV"))     instr = Token.InstrName.InstrEnum.DIV;
		else if (tokenUpper.equals("J"))       instr = Token.InstrName.InstrEnum.J;
		else if (tokenUpper.equals("JAL"))     instr = Token.InstrName.InstrEnum.JAL;
		else if (tokenUpper.equals("JALR"))    instr = Token.InstrName.InstrEnum.JALR;
		else if (tokenUpper.equals("JR"))      instr = Token.InstrName.InstrEnum.JR;
		else if (tokenUpper.equals("LB"))      instr = Token.InstrName.InstrEnum.LB;
		else if (tokenUpper.equals("LH"))      instr = Token.InstrName.InstrEnum.LH;
		else if (tokenUpper.equals("LUI"))     instr = Token.InstrName.InstrEnum.LUI;
		else if (tokenUpper.equals("LW"))      instr = Token.InstrName.InstrEnum.LW;
		else if (tokenUpper.equals("MFHI"))    instr = Token.InstrName.InstrEnum.MFHI;
		else if (tokenUpper.equals("MFLO"))    instr = Token.InstrName.InstrEnum.MFLO;
		else if (tokenUpper.equals("MTHI"))    instr = Token.InstrName.InstrEnum.MTHI;
		else if (tokenUpper.equals("MTLO"))    instr = Token.InstrName.InstrEnum.MTLO;
		else if (tokenUpper.equals("MULT"))    instr = Token.InstrName.InstrEnum.MULT;
		else if (tokenUpper.equals("NOP"))     instr = Token.InstrName.InstrEnum.NOP;
		else if (tokenUpper.equals("NOR"))     instr = Token.InstrName.InstrEnum.NOR;
		else if (tokenUpper.equals("OR"))      instr = Token.InstrName.InstrEnum.OR;
		else if (tokenUpper.equals("ORI"))     instr = Token.InstrName.InstrEnum.ORI;
		else if (tokenUpper.equals("SB"))      instr = Token.InstrName.InstrEnum.SB;
		else if (tokenUpper.equals("SH"))      instr = Token.InstrName.InstrEnum.SH;
		else if (tokenUpper.equals("SLL"))     instr = Token.InstrName.InstrEnum.SLL;
		else if (tokenUpper.equals("SLLV"))    instr = Token.InstrName.InstrEnum.SLLV;
		else if (tokenUpper.equals("SLT"))     instr = Token.InstrName.InstrEnum.SLT;
		else if (tokenUpper.equals("SLTI"))    instr = Token.InstrName.InstrEnum.SLTI;
		else if (tokenUpper.equals("SRA"))     instr = Token.InstrName.InstrEnum.SRA;
		else if (tokenUpper.equals("SRAV"))    instr = Token.InstrName.InstrEnum.SRAV;
		else if (tokenUpper.equals("SRL"))     instr = Token.InstrName.InstrEnum.SRL;
		else if (tokenUpper.equals("SRLV"))    instr = Token.InstrName.InstrEnum.SRLV;
		else if (tokenUpper.equals("SUB"))     instr = Token.InstrName.InstrEnum.SUB;
		else if (tokenUpper.equals("SW"))      instr = Token.InstrName.InstrEnum.SW;
		else if (tokenUpper.equals("SYSCALL")) instr = Token.InstrName.InstrEnum.SYSCALL;
		else if (tokenUpper.equals("XOR"))     instr = Token.InstrName.InstrEnum.XOR;
		else if (tokenUpper.equals("XORI"))    instr = Token.InstrName.InstrEnum.XORI;
		else if (tokenUpper.equals("BGE"))     instr = Token.InstrName.InstrEnum.BGE;
		else if (tokenUpper.equals("BGEZ"))    instr = Token.InstrName.InstrEnum.BGEZ;
		else if (tokenUpper.equals("BGT"))     instr = Token.InstrName.InstrEnum.BGT;
		else if (tokenUpper.equals("BGTZ"))    instr = Token.InstrName.InstrEnum.BGTZ;
		else if (tokenUpper.equals("BLE"))     instr = Token.InstrName.InstrEnum.BLE;
		else if (tokenUpper.equals("BLEZ"))    instr = Token.InstrName.InstrEnum.BLEZ;
		else if (tokenUpper.equals("BLT"))     instr = Token.InstrName.InstrEnum.BLT;
		else if (tokenUpper.equals("BLTZ"))    instr = Token.InstrName.InstrEnum.BLTZ;
		else if (tokenUpper.equals("LA"))      instr = Token.InstrName.InstrEnum.LA;
		else if (tokenUpper.equals("LI"))      instr = Token.InstrName.InstrEnum.LI;
		else if (tokenUpper.equals("MOVE"))    instr = Token.InstrName.InstrEnum.MOVE;
		else 
		{
			// label
