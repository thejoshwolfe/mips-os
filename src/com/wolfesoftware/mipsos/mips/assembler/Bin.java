...
public class Bin
{
	...
	public static class Data extends BinBase
	{
		...
		public static byte[] convertIntegral(Token.TagData.DataTagEnum dataType, long[] nums)
		{
			switch (dataType)
			{
			case BYTE:  return ByteUtils.convertByte(nums);
			case HALF:  return ByteUtils.convertShort(nums);
			case WORD:  return ByteUtils.convertInt(nums);
			case DWORD: return ByteUtils.convertLong(nums);
			default:    return null;
			}
		}
		...
		public static byte[] convertFloating(Token.TagData.DataTagEnum dataType, double[] nums)
		{
			switch (dataType)
			{
			case FLOAT:  return ByteUtils.convertFloat(nums);
			case DOUBLE: return ByteUtils.convertDouble(nums);
			default:     return null;
			}
		}
...
//			return bytes;
//		}
//		// converts an array of numbers to an array of floats stored as an array of bytes
...
//			return bytes;
//		}
//		// converts an array of numbers to an array of doubles stored as an array of bytes
...
//		{
//			byte[] bytes = new byte[(text.length() + 3) & -4]; // pad upto the closest word
...
//		{
//			byte[] bytes = new byte[(text.length() + 4) & -4]; // pad upto the closest word from .length() + 1
//			System.arraycopy(text.getBytes(), 0, bytes, 0, text.length());
//			// bytes[text.length()] = 0;
//			return bytes;
//		}
